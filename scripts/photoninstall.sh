...
setup_docker () {
  ...
  # Enable docker and have it start on boot
...
download_skadi () {
  ...
  sudo chown -R $SKADI_USER:$SKADI_USER /opt/
  sudo chmod -R 766 /opt/
  sudo chmod -R 777 /opt/Skadi/Docker/grafana/
  sudo chmod -R 777 /opt/Skadi/Docker/nginx/
  sudo chmod -R 777 /opt/Skadi/Docker/timesketch/
}
...
start_docker () {
  ...
  echo ""
  cd /opt/Skadi/Docker
  sudo chmod a+rx /opt/Skadi/Docker/grafana/grafana/setup.sh
